...
> & {
  ...
  /**
   * Keep the content mounted (display:none) while closed so opening is a
   * style flip instead of a full mount. Pair with `modal={false}` on Dialog:
   * Radix's modal mode would otherwise lock scroll / aria-hide the app while
   * closed. A plain backdrop is rendered in place of the modal overlay.
   */
  keepMounted?: boolean;
};
...
const DialogContent = React.forwardRef<HTMLDivElement, DialogContentProps>(
  (
    ...
    return (
      <DialogPrimitive.Portal forceMount={keepMounted || undefined}>
        <DialogOverlay />
        {keepMounted ? (
          // Always mounted: Radix portals each child separately, so a
          // backdrop mounted later would land after (above) the content.
          <div
            aria-hidden
            {...scopeProps}
            className={cn(
              "fixed inset-0 z-50 bg-black/40 animate-in fade-in-0 duration-150",
              !open && "hidden",
            )}
          />
        ) : null}
